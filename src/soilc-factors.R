...
xxal$lat_J<-xxal$lat+(rnorm(length(xxal$lat),0,0.00001)) 
...
null.model2<-lme((soil_C)~1,data=xxal,random=~1|dominant)
null.model1<-lme((soil_C)~1,data=xxal,random=~1|dummy)
...
summary(D6)
#
#lm(formula = soil_Ccb ~ abv_C + lat, data = xxal)
#
#Residuals:
#		Min       1Q   Median       3Q      Max 
#-2.54135 -0.76085 -0.05754  0.64515  2.48899 
#
#Coefficients:
#		Estimate Std. Error t value Pr(>|t|)    
#(Intercept) 10.42613    2.38627   4.369 1.83e-05 ***
#		abv_C       -0.03580    0.02997  -1.194  0.23347    
#lat         -0.16149    0.04968  -3.251  0.00131 ** 
#		---
#		Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
#
#Residual standard error: 1.012 on 250 degrees of freedom
#Multiple R-squared:  0.04602,	Adjusted R-squared:  0.03839 
#F-statistic:  6.03 on 2 and 250 DF,  p-value: 0.002769

#soil_C= (soil_Ccb*0.25+1)^4

#Diam_C_model<-lme(log(soil_C) ~ log(abv_C)+lat, data=xxal,random=~1|dominant)
#
#Diam_C_model<-gls(log(soil_C)~ abv_C+lat+elv, data=xxal)
#D1<-gls(soil_C ~ abv_C+lat+elv, data=xxal, correlation = corAR1(,form = ~ 1 | dominant),weights = varPower())
#D2<-gls(soil_C ~ abv_C+lat*long*elv, data=xxal, correlation = corAR1(form = ~ 1 | dominant))
#
#D4<-
#
#
#D3<-gls(soil_C ~ abv_C, data=xxal, correlation = corAR1(form = ~ 1 | dominant))
#AICc(D1,D2,D3)

#bb<-boxcox(soil_C ~ abv_C+lat+elv, data=xxal)
#space<-groupedData(soil_C~abv_C+lat|dominant,data=xxal)
#model4<-gls(soil_C~abv_C+lat,space)
#summary(model4)

#ll<-lm(residuals(Diam_C_model)~lat+long+elv,data=xxal)
#anova(ll)
...
#sense
#All_model<-lme(soil_C~Diam_C*lat_J*long*elv,data=xxal,random=~1|dominant,correlation = corExp(1, form = ~ lat_J + long))
#All_model<-lme(soil_C~abv_C*lat*long*elv,data=xxal)
#
#plot(All_model)
#
...
#MS1<-dredge(All_model,evaluate=T,rank=AICc,trace=T,data=xxal,REML=F)
