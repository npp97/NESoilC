...
# 
# Author: lenovo
# Two-step way to find the chronsequence of forest community after disturnances
# Step 1: DCA and vegclst at whole dataset, then produce several site-groups;
# Step 2: redo step 1 with subgroup data generated by step 1 seperately.
#----------------------------------------------------------------------------------
# Another consideration of dominant species X age X ABG X comm
# Step 1: to inditify the indicator species using  
#
###########################################################################
require(stats);
require(doBy);
...
win.graph()
heatmap(rimp)
##########################Polulation age structure TO indentify the development staus according to 
...
require(mixtools); #the normalmixEM procedure for fitting normal mixture densities
require(flexmix);
require(mixdist);
...
#----------FUzzy clulster
#vgne.fcm = vegclust(rimp, mobileCenters=12,method="FCM", nstart=12)
